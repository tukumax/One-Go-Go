...
# parameters = {'scale_idx':0, 'n_feats':64, 'rgb_range':255, 'n_colors':3, 'scale':[1], 'patch_size':48, 'patch_dim':3,
#               'num_heads':12, 'num_layers':12, 'num_queries':1, 'dropout_rate':0, 'no_mlp':False, 'pos_every':False,
#               'no_pos':False, 'no_norm':False}
parameters = {'scale_idx':3, 'n_feats':64, 'rgb_range':255, 'n_colors':3, 'scale':[2,3,4,1,1,1], 'patch_size':48, 'patch_dim':3,
              'num_heads':12, 'num_layers':12, 'num_queries':6, 'dropout_rate':0, 'no_mlp':False, 'pos_every':False,
              'no_pos':False, 'no_norm':False}
weights, parameters = get_weights_and_parameters(task, parameters)

# load_arch = run_path(os.path.join('basicsr', 'models', 'archs', 'restormer_arch.py'))
# model = load_arch['Restormer'](**parameters)
load_arch = run_path(os.path.join('basicsr', 'models', 'archs', 'ipt_arch.py'))
model = load_arch['ipt'](**parameters)
